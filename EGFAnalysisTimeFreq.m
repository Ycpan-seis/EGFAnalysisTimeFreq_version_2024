...
%Modified: 2018/6/1 USTC
%          Add reference mixed path dispersion for manual and automatic
%Modified: 2022/07/18 USTC
%          Output SNR for each period(global variable g_SNR_T)
%Modified: 2024/07/12 Yichen Pan, USTC, pyc2020@mail.ustc.edu.cn, ycpan03@gmail.com
%          Fixed several bugs; 
%          Improved the automatic mode for higher accuracy and robustness, 
%          and provided a corresponding code for forward 3D surface-wave 
%          phase/group velocity map (SurfMap_CPS).
%          remove the limitation of parameter "Delta T" in automatic mode
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Last Modified by GUIDE v2.5 02-Jun-2018 01:35:49
...
function EGFAnalysisTimeFreq_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = EGFAnalysisTimeFreq_OutputFcn(hObject, eventdata, handles) 
...
load coastlines % the coast line is lat, coastlon,modified by ycpan,2024/6
kk = find(coastlon < 0);
...
if ~get(handles.StopDataProcessing,'Value')
    ...
    clear global g_refphasedisp g_refgroupdisp g_refdispgreatcircle
end
...
global gfcn filter cross g_HighSNRIndex g_SNR_T
global g_PrevStaLonLat
global g_DisperType
global g_refgroupdisp  g_refphasedisp g_refdispgreatcircle
...
cross.NumCtrT = round((cross.EndT - cross.StartT) / cross.DeltaT + 1); % modified by Yichen, 2025/6/16, add "round"
...
NoisePt = round(50/gfcn.SampleT);  %modified by ycpan,2024/9/8, the length depends on your data
...
%SNR_T = zeros(1, cross.NumCtrT); %modified by ycpan,2024/6
...
if IsDispGood == 1   % good data quality
    ...
    if get(handles.GroupVIndex, 'Value')  % also obtain group velocity dispersion curve
        ...
        while IsDispGood == 1
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % add forward mixed path disper modified by myf,2017/11/16
            if ~isempty(g_refdispgreatcircle)
                ref_group_myf=MixedPathDispersion([gfcn.Lat1,gfcn.Lon1],[gfcn.Lat2,gfcn.Lon2],cross.StartT:cross.DeltaT:cross.EndT,...
                    g_refdispgreatcircle.lat_m,g_refdispgreatcircle.lon_m,g_refdispgreatcircle.disperT_m,g_refdispgreatcircle.disp3D_group);
                T_plot=cross.StartT:cross.DeltaT:cross.EndT;
                ref_range=interp1(g_refdispgreatcircle.ref_range(:,1),g_refdispgreatcircle.ref_range(:,2),T_plot);
                hold on;
                plot(T_plot,ref_group_myf,'r','linewidth',2);
                plot(T_plot,ref_group_myf.*(1-ref_range),'g','linewidth',2);
                plot(T_plot,ref_group_myf.*(1+ref_range),'g','linewidth',2);
                hold on
            elseif ~isempty(g_refgroupdisp)
                hold on;
                plot(g_refgroupdisp(:,1),(g_refgroupdisp(:,2)+g_refgroupdisp(:,3))/2,'r','linewidth',2);
                plot(g_refgroupdisp(:,1),g_refgroupdisp(:,2),'g','linewidth',2);
                plot(g_refgroupdisp(:,1),g_refgroupdisp(:,3),'g','linewidth',2);
                hold on
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while IsDispGood == 1
    ...
    % add forward mixed path disper modified by myf,2017/11/16
    if ~isempty(g_refdispgreatcircle)
        ref_phase_myf=MixedPathDispersion([gfcn.Lat1,gfcn.Lon1],[gfcn.Lat2,gfcn.Lon2],cross.StartT:cross.DeltaT:cross.EndT,...
            g_refdispgreatcircle.lat_m,g_refdispgreatcircle.lon_m,g_refdispgreatcircle.disperT_m,g_refdispgreatcircle.disp3D_phase);
        T_plot=cross.StartT:cross.DeltaT:cross.EndT;
        hold on;
        plot(T_plot,ref_phase_myf,'r','linewidth',2);
        plot(T_plot,ref_phase_myf.*(1-ref_range),'g','linewidth',2);
        plot(T_plot,ref_phase_myf.*(1+ref_range),'g','linewidth',2);
        hold on
    elseif ~isempty(g_refphasedisp)
        hold on;
        plot(g_refphasedisp(:,1),(g_refphasedisp(:,2)+g_refphasedisp(:,3))/2,'r','linewidth',2);
        plot(g_refphasedisp(:,1),g_refphasedisp(:,2),'g','linewidth',2);
        plot(g_refphasedisp(:,1),g_refphasedisp(:,3),'g','linewidth',2);
        hold on
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if get(handles.GroupVIndex, 'Value') && GroupTimeIndex == 1  
        ...
        if get(handles.AutoProcessIndex, 'Value')~=1 
            hold on; plot(TPoint(StartTIndex:EndTIndex), gfcn.GroupVDisp(StartTIndex:EndTIndex), 'co', 'MarkerSize', 8);
        end
    end

    % Output the PhaseV image if needed
%     folderName = './fig';
%     if ~exist(folderName, 'dir')
%         mkdir(folderName);
%     end
%     
%     filename = sprintf('%s/%s-%s.png', folderName, gfcn.Name1, gfcn.Name2);
%     exportgraphics(handles.axes2, filename, 'Resolution', 300);
        ...
        if g_refGVwinIndex == 1 || get(handles.GroupVIndex, 'Value');
            ...
            if strcmp(output, 'ref')
                if g_refGVwinIndex == 0
                    ...
                    GroupVwinMin = interp1(g_refWinV(:,1), g_refWinV(:,3),TPoint, 'cubic');
                    GroupVwinMax = interp1(g_refWinV(:,1), g_refWinV(:,4),TPoint, 'cubic');
                end
...
%     set(gca, 'FontSize', 8, 'FontWeight', 'bold','FontName','Arial');   
    ...
    for i = 1:cross.NumCtrT
        ...
        %---------------------------------------------------------------
        % because time - CenterT/8 maybe zero
        %modified by myf 2017/11/17
        site=find(TravPtV==inf);
        for ii=length(site):-1:1
            TravPtV(site(ii))=100;%2*TravPtV(site(ii)+1)-TravPtV(site(ii)+2);
        end
        %---------------------------------------------------------------
...
%     end
    ...
    gfcn.PhaseVImg = interp2(X, Y, fvImage, XI, YI, 'cubic');
    for i = 1:cross.NumCtrT
...
%     end
    ...
    [XI, YI] = meshgrid(TPoint, VPoint);
    gfcn.PhaseVImg = interp2(X, Y, fcimage, XI, YI, 'cubic');
    for i = 1:cross.NumCtrT
...
global g_refgroupdisp gfcn g_refdispgreatcircle
...
% modified by myf 2017/11/17
if ~isempty(g_refdispgreatcircle)
    ref_range=interp1(g_refdispgreatcircle.ref_range(:,1),g_refdispgreatcircle.ref_range(:,2),TPoint);
    gRef =MixedPathDispersion([gfcn.Lat1,gfcn.Lon1],[gfcn.Lat2,gfcn.Lon2],TPoint,...
        g_refdispgreatcircle.lat_m,g_refdispgreatcircle.lon_m,g_refdispgreatcircle.disperT_m,g_refdispgreatcircle.disp3D_group);
    gRef_low=gRef.*(1-ref_range);
    gRef_high =gRef.*(1+ref_range); 
elseif ~isempty(g_refgroupdisp)
    gRef_low = interp1(g_refgroupdisp(:,1), g_refgroupdisp(:,2), TPoint, 'cubic');
    gRef_high = interp1(g_refgroupdisp(:,1), g_refgroupdisp(:,3), TPoint, 'cubic');
else
    error('must give reference dispersion for automatic')
end
gRef_low(gRef_low<gfcn.WinMinV)=gfcn.WinMinV+0.001;
gRef = (gRef_low + gRef_high)/2;
hold on;
plot(TPoint,gRef_low,'g','linewidth',2);hold on;
plot(TPoint,gRef_high,'g','linewidth',2);
plot(TPoint,gRef ,'r','linewidth',2);
%--------------------------------------------------
...
global g_refphasedisp gfcn g_refdispgreatcircle
...
if ~isempty(g_refdispgreatcircle)
    ref_range=interp1(g_refdispgreatcircle.ref_range(:,1),g_refdispgreatcircle.ref_range(:,2),TPoint);
    cRef =MixedPathDispersion([gfcn.Lat1,gfcn.Lon1],[gfcn.Lat2,gfcn.Lon2],TPoint,...
        g_refdispgreatcircle.lat_m,g_refdispgreatcircle.lon_m,g_refdispgreatcircle.disperT_m,g_refdispgreatcircle.disp3D_phase);
    cRef_low=cRef.*(1-ref_range);
    cRef_high =cRef.*(1+ref_range);
elseif ~isempty(g_refphasedisp)
    cRef_low = interp1(g_refphasedisp(:,1), g_refphasedisp(:,2), TPoint, 'cubic');
    cRef_high = interp1(g_refphasedisp(:,1), g_refphasedisp(:,3), TPoint, 'cubic');
else
    error('must give reference dispersion for automatic')
end
cRef_low(cRef_low<gfcn.WinMinV)=gfcn.WinMinV+0.01;
cRef = (cRef_low + cRef_high)/2;
hold on;
plot(TPoint,cRef_low,'g','linewidth',2);hold on;
plot(TPoint,cRef_high,'g','linewidth',2);
plot(TPoint,cRef ,'r','linewidth',2);
%--------------------------------------------------
% find the approximate maximum period for dispersion analysis which
% satisfies interstation_distance > minlamdaRatio*c*T
minlamdaRatio = str2num(get(handles.minWavelength, 'String'));
...
% set trial T and c for searching dispersion curves
T_try_index = round([1 5]/10*(TPoint(nMaxT) - TPoint(1))/dT)+1;  % trial T index for searching dispersion curve
T_try = TPoint(1) + (T_try_index - 1)*dT;  % trial T for searching dispersion index
...
%         ObsSumAbsDiff(i) = sum(abs(diff(PhaseVDisp_try(II(i),1:nMaxT))));        
        % modified by myf,2018/1/12
        disp_temp0=PhaseVDisp_try(II(i),1:nMaxT);
        site=find(disp_temp0>cRef_low(1:nMaxT) &disp_temp0<cRef_high(1:nMaxT)==1);
        if isempty(site)
            site=1;
        end
        disp_temp1=disp_temp0(site);
        ref_disp=(cRef_low(1:nMaxT) + cRef_high(1:nMaxT))/2;
        ref_disp1=ref_disp(site);
        RefObsDispDiff(i) = sum(abs(disp_temp1-ref_disp1))/length(disp_temp1);
        ObsSumAbsDiff(i) = sum(abs(diff(disp_temp1)))/length(disp_temp1);    
    end
    [mindiff, index1] = min(RefObsDispDiff); % find lowest difference dispersion curve with respect to reference
    [minabs, index2] = min(ObsSumAbsDiff); % find smoothest dispersion curve
...
% find reasonable dispersion points with high SNR
%modified by myf,2018/1/16
F=round(1./(NewDisper(2,1)-NewDisper(1,1)));
if F==1
    SNR_gap_num=2;
elseif F==2
    SNR_gap_num=4;
...
for i=1:length(SNRIndex)
    if SNRIndex(i)==0 
        site1=find(SNRIndex(1:i)==1,1);
        site2=find(SNRIndex(i:end)==1,1);
        if ~isempty(site1) && ~isempty(site2) && site2-site1-1<=SNR_gap_num
            SNRIndex(i)=1;
        end
    end
end
...
if DataExistIndex == 1
    ...
    fprintf(ftv,'%f\n', gfcn.Lat2);
    for i = 1:cross.NumCtrT
        fprintf(ftv,'%4.1f    %4.3f    %4.3f   %4d\n',[TPoint(i) VDisp(i)*GoodIndex(i) g_SNR_T(i)  GoodIndex(i)]);
    end
    fclose(ftv); 
    set(gcf,'CurrentAxes',h2);
    hold(h2,'on');
    ...
    set(gcf,'CurrentAxes',h1);
%         hold(h1,'on');
    hold(h1,'off');% modified by myf 2018/1/4
...
% Create smooth range of dcdt. Interpolate to obtain the corresponding period dcdt 
Smooth_r_dcdt = zeros(size(RawDisper,1)-1,3);
Smooth_r_dcdt(:,1) = RawDisper(1:end-1,1);
Smooth_r_dcdt(:,2) = interp1(r_dcdt(:,1),r_dcdt(:,2),Smooth_r_dcdt(:,1),'spline'); %modified 2024/6
Smooth_r_dcdt(:,3) = interp1(r_dcdt(:,1),r_dcdt(:,3),Smooth_r_dcdt(:,1),'spline');
...
if SampleF == 1
    ...
    for ii = 2:size(dcdt,1)-2
        if dcdt(ii,3) == 0.5 && dcdt(ii-1,3)==1
            ...
            %-------------------------------------------------
            % modified by ycpan,2024/6
%             NearestOne=min(QualityAfter==1);
            temp=find(QualityAfter==1);
            if ~isempty(temp)
                NearestOne=temp(1);
            else
                NearestOne=0;
            ...
            %----------------------------------------------
            if sum(QualityAfter(1:NearestOne))/NearestOne > 0.5
                dcdt(ii,3)=1;
            end
        end
    end
...
%-------merge from the first good segment----- (old code)
% TstartIX=find( RawDisper(:,1)==Nonzero(AcceptedSeg(1),1) );
% TendIX=find( abs(RawDisper(:,1)-Nonzero(AcceptedSeg(1),2))<0.0001 ); %modified by ycpan,2024/6
% 
...
%     %TendIX=find( abs(RawDisper(:,1)-Nonzero(AcceptedSeg(1),2))<0.0001 ) %modified by ycpan,2024/6
%     TendIX=find( abs(RawDisper(:,1)-Nonzero(AcceptedSeg(ii),2))<0.0001 ); % Yichen
%     
%     %-----------------------------------------------------------------------------------------
%     % donnot use it modified by myf 2018/1/11 
...
%----merge from the last good segment---- (modified by Yichen, 26.3.1)
% LastSeg = AcceptedSeg(end);
% 
% TstartIX = find(RawDisper(:,1)==Nonzero(LastSeg,1));
% TendIX   = find(abs(RawDisper(:,1)-Nonzero(LastSeg,2))<0.0001);
% 
% NewDisper = RawDisper(TstartIX:TendIX,:);
% 
% for ii = SegNum-1 : -1 : 1
%     
%     CurrSeg = AcceptedSeg(ii);
%     
%     if Nonzero(CurrSeg,1) > MaxTCal - MinTWidth
%         continue;
%     end
%     
%     PrevTstartIX = TstartIX;
%     PrevTendIX   = TendIX;
%     
%     TstartIX = find(RawDisper(:,1)==Nonzero(CurrSeg,1));
%     TendIX   = find(abs(RawDisper(:,1)-Nonzero(CurrSeg,2))<0.0001);
%     
%     PhaseVGap = RawDisper(PrevTstartIX,2) - RawDisper(TendIX,2);
%     TGap = RawDisper(PrevTstartIX,1) - RawDisper(TendIX,1);
%     
%     upper_PhaseVGap1 = mean(dcdt(max(TendIX-4,TstartIX):TendIX-1,2))*TGap*2;
%     upper_PhaseVGap2 = PhaseVGapIntegral(Smooth_r_dcdt,...
%                         RawDisper(TendIX,1),...
%                         RawDisper(PrevTstartIX,1)); 
%                     
%     upper_PhaseVGap = max(upper_PhaseVGap1,upper_PhaseVGap2);
%     lower_PhaseVGap = upper_PhaseVGap2/5
%     
%     if PhaseVGap > upper_PhaseVGap || PhaseVGap < lower_PhaseVGap
%         
%         meanSNR1 = mean(NewDisper(:,3));
...
%         end
%         
%         if meanSNR1 >= meanSNR2 && NewDisper(1,1) > 10
%             break;
%         else
%             NewDisper=[NaN,NaN,NaN];
%         end
%     end
%     
%     NewDisper = [RawDisper(TstartIX:TendIX,:); NewDisper]; 
% end
%--------------------------------------------------------

%-----merge from the longest segment, forward and backward----- (Yichen, 26.3.1)

%[~, idxLongest] = max(Nonzero(AcceptedSeg,3));
%CoreSeg = AcceptedSeg(idxLongest);

% weighted by the SNR and wavelength, use data with the highest confidence as core
% Warning, wavelength threshold is set to be 2, Yichen
scores = zeros(length(AcceptedSeg),1);


for k = 1:length(AcceptedSeg)
    segID = AcceptedSeg(k);
    Tstart = Nonzero(segID,1);
    Tend   = Nonzero(segID,2);
    startIX = find(abs(RawDisper(:,1)-Tstart)<1e-6);
    endIX   = find(abs(RawDisper(:,1)-Tend)<1e-6);
    segT   = RawDisper(startIX:endIX,1);
    segC   = RawDisper(startIX:endIX,2);
    segSNR = RawDisper(startIX:endIX,3);
    % --- SNR---
    snrMask = (segSNR >= minSNR);
    % ---wavelength---
    lambda = segT .* segC;
    geoMask = (2*lambda <= gfcn.StaDist);
    % --- combine ---
    combinedMask = snrMask & geoMask;
    scores(k) = sum(combinedMask);
end

[~, idxLongest] = max(scores);
CoreSeg = AcceptedSeg(idxLongest);

TstartIX = find(abs(RawDisper(:,1)-Nonzero(CoreSeg,1))<1e-6);
TendIX   = find(abs(RawDisper(:,1)-Nonzero(CoreSeg,2))<1e-6);
...
for ii = idxLongest-1 : -1 : 1
    
    CurrSeg = AcceptedSeg(ii);
    
    TstartIX = find(abs(RawDisper(:,1)-Nonzero(CurrSeg,1))<1e-6);
    TendIX   = find(abs(RawDisper(:,1)-Nonzero(CurrSeg,2))<1e-6);
...
for ii = idxLongest+1 : length(AcceptedSeg)
    ...
    upper_PhaseVGap1 = mean(dcdt(max(PrevTendIX-4,PrevTstartIX):PrevTendIX-1,2))*TGap*2;
    upper_PhaseVGap2 = PhaseVGapIntegral(Smooth_r_dcdt,...
                        RawDisper(PrevTendIX,1),...
                        RawDisper(TstartIX,1)); 
...
Smooth_r_dcdt = zeros(size(RawDisper,1)-1,3);
Smooth_r_dcdt(:,1) = RawDisper(1:end-1,1);
Smooth_r_dcdt(:,2) = interp1(r_dcdt(:,1),r_dcdt(:,2),Smooth_r_dcdt(:,1),'linear');
Smooth_r_dcdt(:,3) = interp1(r_dcdt(:,1),r_dcdt(:,3),Smooth_r_dcdt(:,1),'linear');
...
dcdt(:,1)=RawDisper(1:end-1,1);
dcdt(:,2)=RawDisper(2:end,2)-RawDisper(1:end-1,2);
% dcdt(:,2)=(RawDisper(2:end,2)-RawDisper(1:end-1,2))/SampleF;% modified by myf,2018/1/10
...
for ii=1:size(dcdt,1)
    ...
    elseif dcdt(ii,2)>= 1.25*lower_limit && dcdt(ii,2)<=upper_limit*1.25
        dcdt(ii,3)=0.5;
    end
end
...
% elseif SampleF == 2
else %modified by myf 2018/1/11
    for ii = 2:size(dcdt,1)-2
        if dcdt(ii,3)==0 && dcdt(ii-1,3)>0 && dcdt(ii+1,3)>0
            dcdt(ii,3)=0.5;
        end
...
if get(handles.TimeVariableFiltering, 'Value')
    ...
    StartT = str2double(get(handles.StartPeriod,'string'));
    set(handles.minWindowTime, 'string', num2str(5*StartT));
else
...
set(handles.EditSrcStaDist,'String',num2str(round(gfcn.StaDist)));
...
%     % use mouse to click on the dispersion image panel to select start and
%     % end peiord for saving dispersion curve
%     clear xy
%     x = get(h2,'XLim');
%     y = get(h2,'YLim'); 
%     n = 0;
%     set(handles.MsgEdit,'String','Left button click twice on the image to get the start and end period for saving dispersion!');
%     while n < 2
%         [xi,yi] = ginput(1);
%         if xi > x(2) || xi < x(1) || yi > y(2) || yi < y(1)
%             set(handles.MsgEdit, 'String', 'Wrong click! Please click on the lower image!');
...
%                 hold(h2,'on'); plot(h2,[TPoint(StartTIndex),TPoint(StartTIndex)],[y(1),y(2)],'g--');
%             end
%             if n==2
%                 DisperEndT = xi; 
%                 EndTIndex = round((DisperEndT - cross.StartT)/cross.DeltaT) + 1;  
%                 hold(h2,'on'); plot(h2,[TPoint(EndTIndex),TPoint(EndTIndex)],[y(1),y(2)],'g--');
%             end
%         end
%     end
    
  % input start and end period for saving dispersion curve    
    InputOrNotIndex = 1;    
    ...
    for i = 1:cross.NumCtrT
       if (i >= StartTIndex) & (i <=EndTIndex)
           ...
           if StaDistance >= minlamdaRatio*wavelength 
                ...
                fprintf(ftv,'%4.1f    %4.3f    %4.3f   %4d\n',[TPoint(i) DisperV(i) g_SNR_T(i)  1]);
...
%skip the file cannot open, modified by ycpan,2024/6
if fgfcn == -1
    disp(['Skipping file: ' greenfcnfile ' - Unable to open.']);
    maxamp=-1;
    return;
end
...
%     plot(i+4,2+(point_left-1)*0.002,'*k'); hold on;plot(i+4,2+(point_right-1)*0.002,'*k');
...
% --- Executes on button press in RefCGDisperGreatCircle.
function RefCGDisperGreatCircle_Callback(hObject, eventdata, handles)
% hObject    handle to RefCGDisperGreatCircle (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global g_refdispgreatcircle
[pfile1, pname1, index] = uigetfile({'*.mat'},'Open reference dispersion velocity file', pwd);
files = strcat(pname1,pfile1);
g_refdispgreatcircle=load(files);
set(handles.MsgEdit,'String','Reference dispersion read successfully!');
h2 = handles.axes2;
set(gcf,'CurrentAxes',h2);
hold(h2,'off');
plot(g_refdispgreatcircle.ref_range(:,1),g_refdispgreatcircle.ref_range(:,2),'k');
ylim([0 0.2])
xlabel('Period (s)', 'FontSize',8);
ylabel('Ref Velocity Range(%)', 'FontSize',8)
...
% --- Calculate reference dispersion curve
function d=MixedPathDispersion(A,B,T,lat_m,lon_m,disperT_m,phase3D)
% Calculate the mixed path dispersion between points AB.(phase or group velocity
% base on your input)
%
% Input
%   A     [lat,lon]
%   B     [lat,lon]
%   T     perionds you want to calculate
%   lon_m,lat_m,disperT_m,phase3D
%   The sorted 3D dispersion data stores the longitude, latitude, period, 
%   and group velocity/phase velocity dispersion of the corresponding point.
%
% Output
%    d   the mixed path dispersion 
% 
% YafengMeng    2017/11/16
% modified by Yichen Pan 2024/7/16

% A=[30,115];
% B=[35,118];
R=6371;%Radius of the Earth(km)
n=200;%Number of discrete calculate points
[lat,lon]=track2(A(1),A(2),B(1),B(2),[],[],n);% finds the latitude and longitude coordinates of points 
% along a great circle track from the starting point  to the ending point 
distAB=distance(A(1),A(2),B(1),B(2))*pi/180*R;%km
d0=distAB/(n-1);
% d1=distance(lat(1),lon(1),lat(2),lon(2))*pi/180*R;%km
Tv=ones(length(phase3D(1,1,:)),2)*nan;
for i=1:length(phase3D(1,1,:)) 
    lon_i=lon_m(:,:,i);
    lat_i=lat_m(:,:,i);
    disperT_i=disperT_m(1,1,i);
    phase3D_i=phase3D(:,:,i);
    d_i=interp2(lon_i,lat_i,phase3D_i,lon,lat);
     %figure;surf(lon_i,lat_i,phase3D_i);hold on;plot3([A(2),B(2)],[A(1),B(1)],[10,10]);view(2);axis tight;colorbar
    
    t_i=sum(d0./d_i(2:end-1))+(d0./d_i(1)+d0./d_i(end))/2;%The time at both ends is calculated separately
    Tv(i,2)=distAB/t_i;
    Tv(i,1)=disperT_i;
end
d=interp1(Tv(:,1),Tv(:,2),T);
